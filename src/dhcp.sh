...
download_list_domains() {
    ...
    for file in "$TMP_DIR/$FOLDER_NAME/domains/dhcp/"*.txt; do
        cat "$file"
        echo
...
add_domains_to_uci() {
    ...
    # Задаём IP, который будет использоваться для всех доменов
    local domain_ip="94.131.119.85"
    ...
    while IFS= read -r domain; do
        ...
        check_and_add_domain_permanent_name "$domain" "$domain_ip"
